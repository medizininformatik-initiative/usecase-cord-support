...
bundles <- fhir_search(request = search_request, max_bundles = 0, verbose = 2,log_errors = 2)
...
conditions_tmp <- fhir_melt(conditions_raw,
                            columns = c('icd_code','display','system'),
...
conditions_tmp$patient_id <- sub("Patient/", "", conditions_tmp[,5])
conditions_tmp$encounter_id <- sub("Encounter/", "", conditions_tmp[,6])

# merge all patients and conditions data by patient_id
df_merged <- merge(patients_tmp, conditions_tmp, by = "patient_id")

#pseudonymization of patient_id
df_merged$patient_id <- sapply(df_merged$patient_id, digest, algo="md5",serialize=F)

#center infos
df_merged$center_name <- "ZSE" 
df_merged$center_zip <- "11111"
