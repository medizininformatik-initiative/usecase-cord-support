...
if (packageVersion("fhircrackr") >= 2) {
  list_cdn <- fhir_crack(patient_bundle, design_pat, sep = "|", brackets = c("[", "]"), verbose = 2, ncores = 12)
} else {
...
if (exists("token", where = conf)) {
  if (nchar(conf$token) >= 1) {
    condition_bundle <- fhir_search(request = search_request_con, username = conf$user, password = conf$password, token = conf$token, verbose = 2, max_bundles = max_bundles_custom)
    } else {
    condition_bundle <- fhir_search(request = search_request_con, username = conf$user, password = conf$password, verbose = 2, max_bundles = max_bundles_custom)
    }
  } else {
    if (exists("username", where = conf)) {
      condition_bundle <- fhir_search(request = search_request_con, username = conf$user, password = conf$password, verbose = 2, max_bundles = max_bundles_custom)
    } else {
      condition_bundle <- fhir_search(request = search_request_con, verbose = 2, max_bundles = max_bundles_custom)
    }
}

if (packageVersion("fhircrackr") >= 2) {
  list_cdn <- fhir_crack(condition_bundle, design_con, sep = "|", brackets = c("[", "]"), verbose = 2, ncores = 12)
} else {
