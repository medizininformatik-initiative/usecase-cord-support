...
# set age groups
df_conditions_patients$age_group <- cut(df_conditions_patients$age, x, breaks = c(0, 9, 19, 29, 39, 49, 59, 999), labels = c("[1,9]", "[10,19]", "[20,29]", "[30,39]", "[40,49]", "[50,59]", "[60,999]"))
...
df_result_primaer <- as.data.frame(df_cf_birth_all %>% group_by(Einrichtungsindikator = df_cf_birth_all$hospital_id.x, AngabeDiagn1 = df_cf_birth_all$diagnosis.x, AngabeDiagn2 = df_cf_birth_all$diagnosis.y, AngabeGeschlecht = df_cf_birth_all$gender.x, AngabeAlter = df_cf_birth_all$age_group.x) %>% summarise(Anzahl = n()) %>% mutate(Haeufigkeit = paste0(round(100 * Anzahl / sum(Anzahl), 0), "%")))
...
df_result_sekundaer_a <- as.data.frame(df_cf_birth %>% group_by(Einrichtungsindikator = df_cf_birth$hospital_id.x, AngabeDiagn1 = df_cf_birth$diagnosis.x, AngabeDiagn2 = df_cf_birth$diagnosis.y, AngabeGeschlecht = df_cf_birth$gender.x, AngabeAlter = df_cf_birth$age_group.x) %>% summarise(Anzahl = n()) %>% mutate(Haeufigkeit = paste0(round(100 * Anzahl / sum(Anzahl), 0), "%")))
...
df_result_sekundaer_b <- as.data.frame(df_cf_complication %>% group_by(Einrichtungsindikator = df_cf_complication$hospital_id.x, AngabeDiagn1 = df_cf_complication$diagnosis.x, AngabeDiagn2 = df_cf_complication$diagnosis, AngabeGeschlecht = df_cf_complication$gender, AngabeAlter = df_cf_complication$age_group) %>% summarise(Anzahl = n()) %>% mutate(Haeufigkeit = paste0(round(100 * Anzahl / sum(Anzahl), 0), "%")))
