...
# check if maximum bundles parameter is  set
...
# check for custom reference prefix in subject element 
if (exists("subject_element_prefix", where = conf) && nchar(conf$subject_element_prefix) >= 1) {
  subject_element_prefix <- conf$subject_element_prefix
} else {
...
#                            columns = c('diagnosis','system','encounter_id','patient_id'),
#                            brackets = c('[',']'), sep = " || ", all_columns = TRUE,)
...
conditions_tmp$patient_id <- sub(subject_element_prefix, "", conditions_tmp[,"patient_id"])
