...
df_conditions_pku <- subset(df_conditions_tmp, grepl("^E70", diagnosis))
...
df_conditions_pku.0 <- subset(df_conditions_pku, grepl("^E70.0", diagnosis))
#df_conditions_pku.0$recorded_date <- as.Date(df_conditions_pku.0$recorded_date, format= "%Y-%m-%d")
#df_conditions_pku.0 <- df_conditions_pku.0[df_conditions_pku.0$recorded_date < "2019-12-31",]

df_conditions_pku.1 <- subset(df_conditions_pku, grepl("^E70.1", diagnosis))
#df_conditions_pku.1$recorded_date <- as.Date(df_conditions_pku.1$recorded_date, format= "%Y-%m-%d")
#df_conditions_pku.1 <- df_conditions_pku.1[df_conditions_pku.1$recorded_date < "2019-12-31",]

df_conditions_N <- subset(df_conditions_tmp, grepl("^N18", diagnosis))
...
df_pku.0_N <- base::merge(df_conditions_pku.0, df_conditions_N, by = "patient_id")
#df_pku.0_N <- base::merge(df_pku.0_N, df_patients_tmp, by.x = "patient_id",by.y = "patient_id")
#df_pku.0_N <- df_pku.0_N[!duplicated(df_pku.0_N$patient_id),]
#df_pku.0_N$age <- round( as.double( as.Date( df_pku.0_N$recorded_date.y ) - as.Date( df_pku.0_N$birthdate ) ) / 365.25, 0 )
#df_pku.0_N$age_group <- cut(df_pku.0_N$age,x,breaks= c(0,17,30,99), labels = c("[0,17]","[18,30]","[31,99]"))
df_pku.0_N <- df_pku.0_N %>% select(-contains("resource_identifier"))

df_pku.1_N <- base::merge(df_conditions_pku.1, df_conditions_N, by = "patient_id")
#df_pku.1_N <- base::merge(df_pku.1_N, df_patients_tmp, by.x = "patient_id",by.y = "patient_id")
df_pku.1_N <- df_pku.1_N[!duplicated(df_pku.1_N$patient_id),]
#df_pku.1_N$age <- round( as.double( as.Date( df_pku.1_N$recorded_date.y ) - as.Date( df_pku.1_N$birthdate ) ) / 365.25, 0 )
#df_pku.1_N$age_group <- cut(df_pku.1_N$age,x,breaks= c(0,17,30,99), labels = c("[0,17]","[18,30]","[31,99]"))
df_pku.1_N <- df_pku.1_N %>% select(-contains("resource_identifier"))

df_pku_N <- rbind(df_pku.0_N,df_pku.1_N)
